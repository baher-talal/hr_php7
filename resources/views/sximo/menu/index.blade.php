...
<script type="text/javascript" src="{{ asset('sximo/js/plugins/jquery.nestable.js') }}"></script>
 ...
 <p> {{ Lang::get('core.t_tipsnote') }}	</p>
                        ...
                        <button type="submit" class="btn btn-primary"  style="margin-bottom: 100px;">  {{ Lang::get('core.sb_reorder') }} </button>	
		 {!! Form::close() !!}	
		 </div>
		</div>
		<div class="col-sm-7">
				...
				<input type="hidden" name="menu_id" id="menu_id" value="{{ $row['menu_id'] }}" />									
				  ...
				  <div class="form-group  " >
					...
					<div class="col-md-8">
					  ...
					  <a href="{{ URL::to('core/template?show=icons')}}" target="_blank"> Font Awesome </a>  and <a href="{{ URL::to('core/template?show=icon-moon')}}" target="_blank"> Icomoon </a> class name</p>
					 </div> 
				  </div> 					
				  <div class="form-group  " >
